...
| `branchDiscipline`  | `"block" \| "warn" \| "off"`. What happens when work is on `main`, on a stale base, on an already-merged branch, or on a branch a previous sign-off already used. `"warn"` reports, and at commit time asks whether to go ahead anyway; `"block"` refuses. The fading scaffold applied to git: early assignments block, mid-term ones warn, late ones turn it off. |
